...
def get_co2(service, scope):
    ...
    if path.exists():
        with open(
                HOME /
                f'../assets/networking-emissions/{service}/{service}-{fileEnd}.json',
                'r') as file:
            data = json.load(file)
            return data["co2e"]
    else:
...
for name in ['amazon', 'google']:
    with open(HOME / f'../assets/ip-ranges/{name}-ip-ranges.json',
              'r') as file:
        data = json.load(file)
...
with open(HOME / "../assets/ip-ranges/azure-ip-ranges.json", "r") as file:
    ...
    for range in tqdm(azure_ips["values"]):
        ...
        v4_ranges.update({
            ...
            for prefix in properties["addressPrefixes"] if "::" not in prefix
        })
        v6_ranges.update({
            prefix: get_co2("azure", climatiq_region)
            for prefix in properties["addressPrefixes"] if "::" in prefix
        })
